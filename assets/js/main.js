(function ( $ ) {
	...
	$( document ).on(
		'globefarer_trigger_get_new_posts',
		function () {
			qodefSwiper.init();
			qodefMagnificPopup.init();
		}
			...
			return {
				on: {
					...
					init: function () {
						...
						qodef.body.trigger(
							'globefarer_trigger_swiper_is_initialized',
							[$holder, options]
						);
	...
	const qodefCustomAppear = {
		init () {
			const $holders = $('.qodef-custom-appear-right, .qodef-custom-appear-center');

			if ( $holders.length ) {
				$holders.each(
					( index, element ) => {
						qodefCustomAppear.appearAnimation( $( element ) );
					}
				);
			}
		},
	...
	qodef.qodefCustomAppear = qodefCustomAppear;

...
if ( ! function_exists( 'globefarer_get_search_page_excerpt_length' ) ) {
	/**
	 * Function that return number of characters for excerpt on search page
	 *
	 * @return int
	 */
	function globefarer_get_search_page_excerpt_length() {
		$length = apply_filters( 'globefarer_filter_post_excerpt_length', 180 );

		return intval( $length );
	}
}

if ( ! function_exists( 'globefarer_override_search_block_templates' ) ) {
	/**
	 * Function that override `core/search` block template
	 *
	 * @see register_block_core_search()
	 */
	function globefarer_override_search_block_templates( $atts ) {
		if ( ! empty( $atts ) && isset( $atts['render_callback'] ) && 'render_block_core_search' === $atts['render_callback'] ) {
			$atts['render_callback'] = 'globefarer_render_block_core_search';
		}

		return $atts;
	}

	add_filter( 'block_type_metadata_settings', 'globefarer_override_search_block_templates' );
}

if ( ! function_exists( 'globefarer_render_block_core_search' ) ) {
	/**
	 ...
	 */
	function globefarer_render_block_core_search( $attributes ) {
		static $instance_id = 0;

		$attributes = wp_parse_args(
			$attributes,
			array(
				'label'      => esc_html__( 'Search', 'globefarer' ),
				'buttonText' => esc_html__( 'Search', 'globefarer' ),
			)
		);
		...
		if ( $show_button ) {
			...
			if ( ! $use_icon_button ) {
				...
				$button_internal_markup = globefarer_get_svg_icon( 'search' );
			}
